...
AND LEFT(b.kode_layanan_header,3) = 'ORF'", [$request->kodekunjungan]);
    ...
    public function cetakresume($kodekunjungan)
    {
        ...
        $pdf->SetXY($x + 90, $y);
        $pdf->Cell(10, 7, 'RIWAYAT KESEHATAN', 0, 1);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x + 35, $y + 1);
        $pdf->MultiCell(45, 5, '');
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x + 90, $y+5);
        $pdf->Cell(10, 7, 'Riwayat kehamilan bagi pasien wanita', 0, 1);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x + 130, $y + 6);
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x + 1, $y);
        $pdf->MultiCell(70, 5, '1. Apakah pasien mengalami penurunan berat badan yang tidak diinginkan dalam 6 bulan terakhir ?  ');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y + 15);
        $pdf->MultiCell(60, 5, $k[0]->Skrininggizi . ' | ' .  $k[0]->beratskrininggizi);

         $x = $pdf->GetX()+5;
        $y = $pdf->GetY();
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x + 1, $y);
        $pdf->MultiCell(70, 5, '2. Apakah asupan makanan berkurang karena berkurangnya nafsu makan ?');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y + 10);
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x + 1, $y);
        $pdf->MultiCell(70, 5, '3. Pasien dengan diagnosa khusus : Penyakit DM / Ginjal / Hati / Paru / Stroke / Kanker / Penurunan imunitas geriatri, lain lain...');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y + 15);
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x + 1, $y);
        $pdf->MultiCell(70, 5, '4. Bila skor >= 2, pasien beresiko malnutrisi dilakukan pengkajian lanjut oleh ahli gizi');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y + 10);
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x+1, $y);
        $pdf->MultiCell(70, 5, 'Diagnosa Keperawatan');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y+5);
        $pdf->MultiCell(70, 5, $k[0]->diagnosakeperawatan);

        $x = $pdf->GetX()+5;
        $y = $pdf->GetY()+2;
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x+1, $y);
        $pdf->MultiCell(70, 5, 'Rencana Keperawatan/Kebidanan');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y+8);
        $pdf->MultiCell(70, 5,$k[0]->rencanakeperawatan);

        $x = $pdf->GetX()+5;
        $y = $pdf->GetY()+2;
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x+1, $y);
        $pdf->MultiCell(70, 5, 'Tindakan Keperawatan/Kebidanan');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y+5);
        $pdf->MultiCell(70, 5,$k[0]->tindakankeperawatan);

        $x = $pdf->GetX()+5;
        $y = $pdf->GetY()+2;
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetXY($x+1, $y);
        $pdf->MultiCell(60, 5, 'Evaluasi Keperawatan/Kebidanan');
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY($x, $y+6);
        $pdf->MultiCell(40, 1,$k[0]->evaluasikeperawatan);
        $pdf->Output();
        exit;
    }
