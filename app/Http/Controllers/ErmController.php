...
use App\Models\ts_konsul_antar_poli;
...
class ErmController extends Controller
{
    ...
    public function riwayattindakan2(Request $request)
    {
        ...
        RIGHT OUTER JOIN mt_tarif_header e ON d.`KODE_TARIF_HEADER` = e.`KODE_TARIF_HEADER`
        WHERE a.`kode_kunjungan` = ?
        AND substr(b.kode_unit,1,1) = 1 AND c.kode_tarif_detail NOT IN('TX50683','TX50153','TX45113','TX50263','TX46883')", [$request->kodekunjungan]);
            ...
            try {
                ...
                foreach ($arrayindex_tindakan as $d) {
                    ...
                    $save_detail = [
                        ...
                        'total_layanan' => $total_tarif,
                        'grantotal_layanan' => $total_tarif,
                        'kode_dokter1' => $request->kodeparamedis,
                        'status_layanan_detail' => 'OPN',
                        'tgl_layanan_detail' => $now,
                        'tagihan_penjamin' => $tagihanpenjamin,
                        'tagihan_pribadi' => $tagihanpribadi,
    ...
    public function riwayatkonsul(Request $request)
    {
        ...
        $data2 = DB::select('select *,fc_nama_unit1(unit_tujuan) as nama_poli_tujuan, fc_nama_paramedis1(dokter_pengirim) as nama_dokter_pengirim from ts_konsul_antar_poli where kode_kunjungan = ?', [$kodekunjungan]);
        return view('ermtemplate.tabel_riwayat_konsul', compact([
            'data2'
        ]));
    }
    public function batalkonsull(Request $request)
    {
        $id = $request->id;
        ts_konsul_antar_poli::where('id', $id)->delete();
        $data = [
            'kode' => 200,
    ...
    public function ambil_view_form_konsul(Request $request)
    {
        $kodekunjungan = $request->kodekunjungan;
        $mt_unit = db::select('select * from mt_unit where substr(kode_unit,1,1) = 1');
        return view('ermdokter.view_form_konsul',compact([
            'mt_unit'
        ]));
    }
    public function simpankonsul_baru(Request $request)
    {
        $kode_unit = $request->kodeunit;
        $kodekunjungan = $request->kodekunjungan;
        $kunjungan = db::select('select * from ts_kunjungan where kode_kunjungan = ?',[$kodekunjungan]);
        foreach($kode_unit as $k){
            $datakonsul = [
                'no_rm' => $kunjungan[0]->no_rm,
                'kode_kunjungan' => $request->kodekunjungan,
                'unit_pengirim' => $kunjungan[0]->kode_unit,
                'unit_tujuan' => $k,
                'catatan' => $request->catatankonsul,
    ...
    public function simpanrujin_baru(Request $request)
    {
        $kode_unit = $request->kodeunit;
        $kodekunjungan = $request->kodekunjungan;
        $kunjungan = db::select('select * from ts_kunjungan where kode_kunjungan = ?',[$kodekunjungan]);
    ...
    public function hasilpemeriksaanmedis(Request $request)
    {
        $kodekunjungan = $request->kodekunjungan;
        // $assesmen_dokter = db::select('select *,tgl_kunjungan as tglk from assesmen_dokters where id_kunjungan = ?',[$kodekunjungan]);
        $datakonsul = db::select('select *,fc_nama_unit1(unit_pengirim) as poli_pengirim,fc_nama_unit1(unit_tujuan) as poli_konsul,fc_nama_paramedis1(dokter_penerima) as dokter_penerima_2 from ts_konsul_antar_poli where kode_kunjungan = ?', [$kodekunjungan]);
        ...
        WHERE a.`kode_kunjungan` = ? AND a.`kode_unit` < ?',[$kodekunjungan,'4000']);
         ...
         $assesmen_dokter = DB::connection('mysql')->select('SELECT *,a.id as idasskep,b.versi as versidk,date(a.tanggalkunjungan) as tglk,a.kode_unit as unitpoli ,fc_nama_unit1(a.kode_unit) as nama_unit FROM erm_hasil_assesmen_keperawatan_rajal a LEFT OUTER JOIN assesmen_dokters b on a.kode_kunjungan = b.id_kunjungan LEFT OUTER JOIN ts_kunjungan c on a.kode_kunjungan = c.kode_kunjungan
             WHERE a.kode_kunjungan = ? and c.status_kunjungan != 8 and a.jenis_berkas = ? order  by a.id desc', [$kodekunjungan, 2]);
    ...
    public function simpankonsulantarpoli(Request $request)
    {
        $data = json_decode($_POST['data'], true);
        foreach ($data as $nama) {
            $index =  $nama['name'];
            ...
            $cek_data_konsul = db::select('select * from ts_konsul_antar_poli where kode_kunjungan = ? and jenis = ?',[$request->kodekunjungan,'KONSUL']);
            if(count($cek_data_konsul) == 0){
                $kunjungan = db::select('select * from ts_kunjungan where kode_kunjungan = ?',[$request->kodekunjungan]);
                $datakonsul = [
                'no_rm' => $kunjungan[0]->no_rm,
                'kode_kunjungan' => $request->kodekunjungan,
                'unit_pengirim' => $kunjungan[0]->kode_unit,
                'unit_tujuan' => $dataSet['politujuankonsul'],
                'catatan' => $dataSet['catatankonsul'],
                'dokter_pengirim' => $dataSet['kodedokterpengirimkonsul'],
                'tgl_konsul' => $dataSet['tanggalkonsul'],
                'jenis' => 'KONSUL',
                'kode_kunjungan_2' => 0,
                'status' => 0
                ];
    ...
    public function simpanrujukinternal(Request $request)
    {
        ...
        foreach ($data as $nama) {
            ...
            $cek_data_konsul = db::select('select * from ts_konsul_antar_poli where kode_kunjungan = ? and jenis = ?',[$request->kodekunjungan,'RUJIN']);
            if(count($cek_data_konsul) == 0){
                $kunjungan = db::select('select * from ts_kunjungan where kode_kunjungan = ?',[$request->kodekunjungan]);
                $datakonsul = [
                'no_rm' => $kunjungan[0]->no_rm,
                'kode_kunjungan' => $request->kodekunjungan,
                'unit_pengirim' => $kunjungan[0]->kode_unit,
                'unit_tujuan' => $dataSet['namapolirujin'],
                'catatan' => $dataSet['catatanrujin'],
                'dokter_pengirim' => $dataSet['kodedokterrujin'],
                'tgl_konsul' => $dataSet['tanggalrujin'],
                'jenis' => 'RUJIN',
                'kode_kunjungan_2' => 0,
                'status' => 0
                ];
    ...
    public function ambilriwayatsuratrujin(Request $request)
    {
        $kodekunjungan = $request->kodekunjungan;
        $datakonsul = db::select('select *,fc_nama_unit1(unit_pengirim) as poli_pengirim,fc_nama_unit1(unit_tujuan) as poli_konsul,fc_nama_paramedis1(dokter_penerima) as dokter_penerima_2 from ts_konsul_antar_poli where kode_kunjungan = ?', [$kodekunjungan]);
        return view('ermtemplate.tabel_riwayat_rujin',compact([
            'datakonsul'
        ]));
    }
}
