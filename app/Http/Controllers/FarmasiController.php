...
class FarmasiController extends Controller
{
    ...
    public function datakunjungan_farmasi()
    {
        $title = 'SIMRS - DATA KUNJUNGAN FARMASI';
        $sidebar = 'datakunjungan_farmasi';
        $sidebar_m = 'datakunjungan_farmasi';
        $mt_unit = DB::select('select * from mt_unit where group_unit = ?', (['J']));
        $now = $this->get_date();
        $now2 = Carbon::now();
        $oneweek = $now2->startOfWeek()->format('Y-m-d');
        return view('farmasi.index_data_kunjungan', compact([
            'title',
            'sidebar',
            'sidebar_m',
            'now',
