<div class="card">
    <div class="card-header bg-light text-bold">
        @if(count($mt_pasien) > 0)
        {{ $mt_pasien[0]->no_rm }} | {{ $mt_pasien[0]->nama_px }} | {{ \Carbon\Carbon::parse($mt_pasien[0]->tgl_lahir)->format('Y-m-d') }}
        (Usia {{ \Carbon\Carbon::parse($mt_pasien[0]->tgl_lahir)->age }}) | Alamat : {{ $mt_pasien[0]->alamatpasien }}
        @else
        <h3 class="text-danger">Pasien Tidak ditemukan ! Masukan 8 digit nomor rekamedis pasien ...</h3>
        @endif
        </div>
    <div class="card-body">
