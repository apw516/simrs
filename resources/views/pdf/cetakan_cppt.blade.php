            ...
            <div class="isi-surat">
                <table class="mt-2 " style="border: 1px solid;width:100%">
                    <tbody>
                        ...
                        <tr>
                            <td style="text-align: center;height: 1px;" colspan="2" class="text-bold">ASSESMEN AWAL
                            </td>
                            <td style="font-style: italic;font-size:8px;margin-top:14px" colspan="2"
                                class="text-bold">
                        ...
                        <tr>
                            <td style="text-align: left;height: 1px;" colspan="4" class="text-bold">POLI KLINIK :
                                {{ $header[0]->NAMAUNIT }} | Tanggal Periksa
                                :{{ \Carbon\Carbon::parse($header[0]->tanggalkunjungan)->format('d / M / Y') }}</td>
                        </tr>
                        <tr>
                            <td width="50%" style="text-align: left;height: 1px;" colspan="1" class="text-bold">
                                ASSESMEN AWAL
                                KEPERAWATAN</td>
                            <td width="50%" style="text-align: left;height: 1px;" colspan="3" class="text-bold">
                                ASSESMEN AWAL MEDIS
                            </td>
                        </tr>
                        <tr>
                            <td style="font-size:8px;text-align: left;height: 1px;vertical-align: top;" colspan="1">
                            ...
                            <td style="font-size:8px;text-align: left;height: 1px;font-size:12px" colspan="3">
                                @if ($header[0]->kode_unit != 1028)
                                    ...
                                    Riwayat Alergi : {{ $header[0]->riwayat_alergi }} |
                                    {{ $header[0]->keterangan_alergi }}<br>
                                    ...
                                    Tindak Lanjut :
                                    {{ $header[0]->tindak_lanjut }}<br>{{ $header[0]->keterangan_tindak_lanjut }}<br><br>
                                    ...
                                    @foreach ($orderfarmasi as $of)
                                        @if ($of->kode_kunjungan == $header[0]->kode_kunjungan)
                                            {{ $of->kode_barang }} {{ $of->keteranganresep }} | qty :
                                            {{ $of->jumlah_layanan }} | {{ $of->aturan_pakai }}<br>
                                        @endif
                                    @endforeach
                                    <br>
                                    <br>
                        ...
                        <tr class="page-break-row">
                            <td colspan="1" style="text-align: center; vertical-align: bottom;">
                                <br>
                                <img src="data:image/svg+xml;base64,{{ $qrtteaskep }}"
                                    style="width: 60px; height: 60px; display: block;" alt="QR Code TTE">
                                <br>
                                <br>Pemeriksa : {{ $header[0]->namapemeriksa }}
                            </td>
                            <td colspan="3" style="text-align: center; vertical-align: bottom;">
                                <br>
                                <img src="data:image/svg+xml;base64,{{ $qrtteasmed }}"
                                    style="width: 60px; height: 60px; display: block;" alt="QR Code TTE">
                                <br>
                                <br>Dokter pemeriksa : {{ $header[0]->nama_dokter }}
                            </td>
                        </tr>
                    </tbody>
                </table>
                            ...
                            @if ($header[0]->kode_kunjungan == $cp->id_header || $header[0]->kode_kunjungan == $cp->ref_kunjungan)
                                <tr>
                                    <td style="vertical-align: top;">
                                        {{ \Carbon\Carbon::parse($cp->tglk)->format('d / M / Y') }}</td>
                                    <td width="40%" style="font-size:8px;vertical-align:top">
                                        @if ($cp->kode_unit != 1028)
                                            ...
                                            Evaluasi Keperawatan : {{ $cp->evaluasikeperawatan }}<br><br>

                                            <!-- Barcode / QR TTE Pemeriksa -->
                                            @foreach ($data_pemeriksa as $ddd)
                                                @if ($ddd['id_header'] == $cp->idasskep)
                                                    <img src="data:image/svg+xml;base64,{{ $ddd['TTE_PERAWAT'] }}"
                                        ...
                                        @else
                                            Hasil Pemeriksaan : {{ $cp->tindakankeperawatan }}<br><br>
                                            @foreach ($data_pemeriksa as $ddd)
                                                @if ($ddd['id_header'] == $cp->idasskep)
                                                    <img src="data:image/svg+xml;base64,{{ $ddd['TTE_PERAWAT'] }}"
                                                        style="width: 60px; height: 60px; display: block;"
                                    ...
                                    <td width="40%" style="font-size:8px;vertical-align:top">
                                        @if ($cp->kode_unit != 1028)
                                            ...
                                            Riwayat Penyakit Dahulu : {{ $cp->riwayat_kehamilan_pasien_wanita }} <br>
                                            {{ $cp->riwyat_kelahiran_pasien_anak }} <br>
                                            {{ $cp->riwyat_penyakit_sekarang }} <br>

                                            Riwayat Alergi : {{ $cp->riwayat_alergi }} |
                                            {{ $cp->keterangan_alergi }}<br>
                                            ...
                                            Obat Obatan : Order yang dikirim<<br>
                                                ...
                                                @endforeach
                                                <br>
                                                Jawaban Konsul Ke poli lain :
                                                {{ $cp->keterangan_tindak_lanjut_2 }}<br><br>
                                                @foreach ($datakonsul as $dk)
                                            ...
                                            @else
                                                ...
                                                Obat Obatan : Order yang dikirim<<br>
                                                    @foreach ($orderfarmasi as $of)
                                                        @if ($of->kode_kunjungan == $cp->kode_kunjungan)
                                                            {{ $of->kode_barang }} | {{ $of->keteranganresep }} |
                                                            qty :{{ $of->jumlah_layanan }} | {{ $of->aturan_pakai }}
                                                            <br>
                                                        @endif
                                                    @endforeach
                                                    <br>
                                    ...
                                    <td>
                                        @foreach ($data_pemeriksa as $ddd)
                                            @if ($ddd['id_header'] == $cp->idasskep)
                                                <img src="data:image/svg+xml;base64,{{ $ddd['TTE_DOKTER'] }}"
                                                    style="width: 60px; height: 60px; display: block;"
                                                    alt="QR Code TTE"><br>
                                            @endif
                                        @endforeach
