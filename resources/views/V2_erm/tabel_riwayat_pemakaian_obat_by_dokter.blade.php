@foreach ($HEADER as $kl)
    <div class="card">
        <div class="card-header bg-info">{{ $kl->unit_pengirim }} | {{ $kl->tgl_entry }} | {{ $kl->nama_dokter }} <button
                class="btn btn-warning float-right text-bold pilihresepdokter" idheader="{{ $kl->id }} " kodekunjungan="{{ $kl->kode_kunjungan}}"><i
...
@endforeach
<script>
    $(".pilihresepdokter").on('click', function(event) {
        ...
        $.ajax({
            ...
            url: '<?= route('v2_add_riwayat_pemakaian_obat_by_dokter') ?>',
            error: function(data) {
                spinner.hide()
                Swal.fire({
                    icon: 'error',
